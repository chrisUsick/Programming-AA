...
# Conclusion
Autonomous agents are entities which interact with their environment. Autonomous agents can be classified based on their abilities and intelligence. There are many real-world situations which use autonomous agents. Actuators, sensors and state are the components which make up an AA. AA Factory, an interactive programming tool, was used to implement an autonomous agent. When implementing an autonomous agent there are several considerations about the environment and the agent being implemented that should be made. One must consider the properties of the environment as well as the nature of the agent. This involves determining how can it perceive the world, and how does it interact with it.   
An autonomous agent was implemented which successfully completed the task of seeking out targets and destroying them in an environment where the autonomous agent wasn't able to perceive the whole world at once. With AA Factory, it took less than 40 lines of code to implement an autonomous agent which performed a non-trivial task. It is clear that through AA Factory autonomous agent development can be explored in an intuitive and simple way. Autonomous agents are a science that will change the world, and by learning about them one may appreciate, and even contribute to the future of autonomous agents.
